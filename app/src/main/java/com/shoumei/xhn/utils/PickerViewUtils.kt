...
class PickerViewUtils(var context: Context) {
    ...
    fun showYmd(onYmdResult: (dateStr: String, timestamp: String) -> Unit) {
        ...
        val datePickerDialog = DatePickerDialog(context, DatePickerDialog.THEME_HOLO_LIGHT,
                DatePickerDialog.OnDateSetListener { _: DatePicker, year: Int, month: Int, dayOfMonth: Int ->
                    calendar.set(year, month, dayOfMonth)
                    val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
                    onYmdResult(dateFormat.format(calendar.time), (calendar.time.time / 1000).toString())
                },
